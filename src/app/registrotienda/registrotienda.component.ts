...
export class RegistrotiendaComponent implements OnInit {
  ...
  private readonly regTiendaSvc:RegistrotiendaService, private toastr:ToastrService ) { }
...
//-------------------metodo para agregar a bd -----
    ...
    this.crudTiendas.createstore(record).then((resp)=>{
      ...
      this.toastr.success('tienda creado correctamente','Exito')
  
      })
      .catch((err)=>{
        this.toastr.error('no se puedo crear la tienda','Error')
      })
